  ...
  return (
    <main className="mx-auto max-w-5xl px-6 py-10">
      ...
      <section className="mt-8 rounded-2xl border border-gray-200 p-6">
        ...
        {loaded && <div className="mt-6 rounded-xl bg-emerald-50 p-4"><div className="flex items-center justify-between gap-4 text-sm"><span className="font-semibold text-emerald-950">{language === "zh" ? `路线进度：${completion.completed}/${completion.total} 门` : `Path progress: ${completion.completed}/${completion.total}`}</span><span className="font-bold text-emerald-900">{completion.percent}%</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-emerald-100"><div className="h-full rounded-full bg-emerald-600" style={{ width: `${completion.percent}%` }} /></div><p className="mt-2 text-xs text-emerald-800">{language === "zh" ? "登录后可跨设备同步；游客记录保存在当前浏览器。重复选修课只计算一次。" : "Sign in for cross-device sync; guest records stay in this browser. Repeated electives count once."}</p></div>}
