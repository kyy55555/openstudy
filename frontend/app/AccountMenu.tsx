...
export default function AccountMenu() {
  ...
  return <nav aria-label={language === "zh" ? "用户快捷操作" : "Account shortcuts"} className="fixed bottom-[max(1.25rem,env(safe-area-inset-bottom))] left-4 right-4 z-50 flex flex-wrap items-center justify-end gap-2"><ThemeToggle language={language} /><Link href={language === "zh" ? "/dashboard?lang=zh" : "/dashboard"} className="rounded-full border border-gray-200 bg-white px-4 py-2 text-sm font-medium shadow-lg hover:border-gray-400">{language === "zh" ? "用户中心" : "User center"}</Link><Link href={href} className="rounded-full border border-gray-200 bg-white px-4 py-2 text-sm font-medium shadow-lg hover:border-gray-400">{label}</Link></nav>;
}
