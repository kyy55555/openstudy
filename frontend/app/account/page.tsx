...
function AccountContent() {
  ...
  async function requestPasswordReset() {
    if (!client || !email) {
      setMessage(language === "zh" ? "请先填写邮箱。" : "Enter your email first.");
      return;
    }
    setBusy(true);
    setMessage("");
  ...
  async function updatePassword(event: React.FormEvent) {
    ...
    if (error) setMessage(error.message);
      ...
      setPassword("");
      setMessage(language === "zh" ? "密码已更新，可以继续使用当前账号。" : "Password updated. You can continue with this account.");
    }
  }
  ...
  return <main className="mx-auto min-h-screen max-w-xl px-6 py-12">
    ...
    </section> : recoveryMode ? <section className="mt-8 rounded-2xl border border-blue-200 bg-blue-50 p-6">
