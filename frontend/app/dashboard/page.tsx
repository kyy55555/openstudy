...
import { createCourseLibraryBackup, learningPathCoverage, parseCourseLibraryBackup } from "../../data/courseLibrary";
...
function DashboardContent() {
  ...
  const { library, loaded, syncIssue, toggleStudyTask, recordResourceOpen, clearLastOpenedResource, replaceLibrary } = useCourseLibrary();
  ...
  return <main className="mx-auto min-h-screen max-w-6xl px-6 py-10">
    ...
    <section className="mt-8 rounded-2xl border border-gray-200 p-5"><h2 className="text-xl font-semibold">{copy.backup}</h2><p className="mt-2 text-sm text-gray-600">{copy.backupHelp}</p><div className="mt-4 flex flex-wrap gap-3"><button type="button" onClick={downloadBackup} className="rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium hover:border-black">{copy.download}</button><label className="cursor-pointer rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium hover:border-black">{copy.chooseBackup}<input type="file" accept="application/json,.json" onChange={chooseBackup} className="sr-only" /></label></div>{pendingBackup && <div className="mt-4 rounded-xl border border-amber-300 bg-amber-50 p-4"><p className="text-sm font-semibold text-amber-950">{copy.backupFrom}：{new Date(pendingBackup.exportedAt).toLocaleString(language === "zh" ? "zh-CN" : "en-US")}</p><p className="mt-1 text-xs text-amber-900">{copy.courses}：{Object.values(pendingBackup.library.progress).filter((status) => status === "completed").length} · {copy.resources}：{pendingBackup.library.completedResources.length} · {copy.plans}：{Object.keys(pendingBackup.library.studyPlans).length}</p><div className="mt-3 flex gap-2"><button type="button" onClick={restoreBackup} className="rounded-lg bg-amber-950 px-3 py-2 text-sm font-medium text-white">{copy.restore}</button><button type="button" onClick={() => setPendingBackup(null)} className="rounded-lg border border-amber-400 px-3 py-2 text-sm font-medium text-amber-950">{copy.cancel}</button></div></div>}{backupMessage && <p role="status" className="mt-3 text-sm text-gray-700">{backupMessage}</p>}</section>
  </main>;
}
