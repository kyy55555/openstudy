...
function DashboardContent() {
  ...
  const { library, loaded, toggleStudyTask } = useCourseLibrary();
  ...
  return <main className="mx-auto min-h-screen max-w-6xl px-6 py-10">
    ...
    <section className="mt-8"><h2 className="text-xl font-semibold">{copy.continue}</h2>{activePlans.length === 0 ? <p className="mt-3 text-sm text-gray-500">{copy.noPlan}</p> : <div className="mt-3 grid gap-4 lg:grid-cols-2">{activePlans.map(({ course, saved, generated, nextTask }) => <div key={course.id} className="rounded-2xl border border-violet-200 bg-violet-50 p-5"><Link href={courseDetailPath(course, language)} className="font-semibold hover:underline">{courseCode(course)} · {language === "zh" ? course.titleZh : course.title}</Link><p className="mt-1 text-xs text-violet-800">{copy.target} {saved.days} · {copy.planned} {generated.plannedDays}</p>{nextTask ? <><a href={nextTask.url} target="_blank" rel="noreferrer" className="mt-4 block text-sm font-medium hover:underline">{copy.next}：{language === "zh" ? nextTask.titleZh : nextTask.title} ↗</a><button onClick={() => toggleStudyTask(course.id, nextTask.id)} className="mt-3 rounded-lg bg-violet-900 px-3 py-2 text-sm text-white">{copy.completeTask}</button></> : <p className="mt-4 text-sm text-emerald-800">{copy.finishedPlan}</p>}</div>)}</div>}</section>
