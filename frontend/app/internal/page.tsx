...
function InternalContent() {
  ...
  const copy = language === "zh" ? { title: "OpenStudy 内部数据", subtitle: "产品使用漏斗、注册账号与用户反馈。仅管理员可见。", account: "前往账号登录", refresh: "刷新数据", users: "注册账号", visitors: "30 天产品访客", courses: "打开课程", resources: "打开资料", plans: "创建计划", tasks: "完成任务", feedback: "用户反馈", noFeedback: "目前还没有反馈。", page: "提交页面", reply: "回复邮箱", generated: "数据生成时间" } : { title: "OpenStudy internal data", subtitle: "Product funnel, registered accounts, and user feedback. Administrators only.", account: "Sign in to account", refresh: "Refresh data", users: "Registered accounts", visitors: "30-day product visitors", courses: "Opened courses", resources: "Opened resources", plans: "Created plans", tasks: "Completed tasks", feedback: "User feedback", noFeedback: "No feedback yet.", page: "Submitted from", reply: "Reply email", generated: "Generated" };
  ...
  return <main className="mx-auto min-h-screen w-full max-w-6xl px-4 pb-32 pt-8 sm:px-6 sm:py-12">
    ...
    {data && <>
      <section className="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">{[[copy.users, data.userCount], [copy.visitors, data.funnel30d.active_visitors ?? 0], [copy.courses, data.funnel30d.opened_course ?? 0], [copy.resources, data.funnel30d.opened_resource ?? 0], [copy.plans, data.funnel30d.created_plan ?? 0], [copy.tasks, data.funnel30d.completed_task ?? 0]].map(([label, value]) => <div key={String(label)} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><p className="text-xs text-gray-500">{label}</p><p className="mt-2 text-3xl font-bold">{value}</p></div>)}</section>
