...
import { localDateKey } from "../../data/courseLibrary";
...
function TodayContent() {
  ...
  const { library, loaded, syncIssue, syncConflict, completeDailyTask, recordResourceOpen } = useCourseLibrary();
  ...
  const copy = language === "zh" ? {
    back: "← OpenStudy", center: "用户中心", title: "今日学习", subtitle: "今天只完成这一小步。完成后不再追加任务。", task: "今日任务", done: "今天的学习完成了", doneHelp: "进度已经保存。明天会从下一项继续。", empty: "还没有今日任务。先选择一门课程并制定学习计划。", browse: "浏览课程", manage: "管理课程计划", open: "打开官方内容", complete: "完成今天的任务", saved: "完成啦！今天的学习进度已经保存。", progress: "课程计划进度", next: (current: number, total: number) => `接下来是第 ${current} 项，共 ${total} 项`, sync: "云同步暂时不可用，完成记录会先保存在此设备。", conflict: "发现其他设备的更新，请先到用户中心选择保留哪份记录。",
  } : {
  ...
  return (
    <main className="mx-auto min-h-screen max-w-5xl px-5 py-8 sm:px-6 sm:py-10">
      ...
      <section className="mt-8 overflow-hidden rounded-3xl bg-gradient-to-br from-violet-950 via-violet-900 to-indigo-700 p-6 text-white shadow-xl shadow-violet-950/15 sm:p-9">
