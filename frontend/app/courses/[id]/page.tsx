  ...
  return (
    <main className="mx-auto max-w-4xl px-6 py-10">
      ...
      <section className="mt-8 rounded-2xl border border-gray-200 p-7 shadow-sm">
        ...
        {loaded && planDefinition && <section className="mt-8 rounded-2xl border border-violet-200 bg-violet-50/50 p-5"><h2 className="text-xl font-semibold text-violet-950">{language === "zh" ? "弹性学习计划" : "Flexible study plan"}</h2><p className="mt-2 text-sm text-violet-900">{language === "zh" ? "输入你希望完成课程的天数。由于规划是估算，系统会保守增加约 15% 的缓冲时间，生成的计划绝不会短于你的目标。未完成内容不会与下一天叠加。" : "Enter your target. Because planning is an estimate, OpenStudy adds about 15% buffer time and never creates a plan shorter than your target. Unfinished work does not stack onto the next day."}</p>
