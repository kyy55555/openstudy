...
import { feedbackAppVersion, feedbackPrefill, feedbackViewport, isFeedbackIssueType, legacyFeedbackMessage, shouldRetryLegacyFeedback, type FeedbackIssueType } from "../../data/feedback";
...
function FeedbackContent() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    setBusy(true);
    setStatus("");
    const common = { message: legacyFeedbackMessage(issueType, message), email: email.trim() || null, page_url: window.location.href };
    const structured = {
      ...common,
      issue_type: issueType,
      language,
      viewport: feedbackViewport(window.innerWidth),
      app_version: feedbackAppVersion(process.env.NEXT_PUBLIC_OPENSTUDY_COMMIT),
    };
  ...
  return <main className="mx-auto min-h-screen max-w-xl px-6 py-12">
    ...
    <form onSubmit={submit} className="mt-8 space-y-5">
      ...
      <label className="block text-sm font-medium">{language === "zh" ? "问题类型" : "Issue type"}<select value={issueType} onChange={(event) => { if (isFeedbackIssueType(event.target.value)) setIssueType(event.target.value); }} className="mt-2 block w-full rounded-xl border border-gray-300 bg-white p-3"><option value="broken-link">{language === "zh" ? "链接失效或跳转错误" : "Broken or incorrect link"}</option><option value="course-data">{language === "zh" ? "课程信息不准确" : "Incorrect course information"}</option><option value="curriculum-data">{language === "zh" ? "培养方案不准确" : "Incorrect curriculum reference"}</option><option value="missing-course">{language === "zh" ? "希望增加课程" : "Request a course"}</option><option value="study-plan">{language === "zh" ? "学习计划不合理" : "Study-plan problem"}</option><option value="account-sync">{language === "zh" ? "账号或云同步问题" : "Account or cloud-sync problem"}</option><option value="mobile-accessibility">{language === "zh" ? "手机或无障碍问题" : "Mobile or accessibility problem"}</option><option value="translation">{language === "zh" ? "翻译问题" : "Translation issue"}</option><option value="account-deletion">{language === "zh" ? "删除账号与云端记录" : "Delete account and cloud record"}</option><option value="other">{language === "zh" ? "其他问题或建议" : "Other problem or suggestion"}</option></select></label>
