...
const tsinghuaCombinatoricsTasks: PlanTask[] = tsinghuaCombinatoricsWeeks.flatMap(([title, titleZh], index) => [{ id: `week-${index + 1}`, title: `Official week ${index + 1}: ${title}`, titleZh: `官方第 ${index + 1} 周：${titleZh}`, url: tsinghuaCombinatoricsSource, kind: "session" as const }, { id: `homework-${index + 1}`, title: `Official week ${index + 1} homework`, titleZh: `官方第 ${index + 1} 周作业`, url: tsinghuaCombinatoricsSource, kind: "assignment" as const }]);
...
const tsinghuaAlgorithmTasks: PlanTask[] = tsinghuaAlgorithmUnits.flatMap(([title, titleZh], index) => [{ id: `unit-${index + 1}`, title: `Official unit ${index + 1}: ${title}`, titleZh: `官方单元 ${index + 1}：${titleZh}`, url: tsinghuaAlgorithmSource, kind: "session" as const }, { id: `homework-${index + 1}`, title: `Official homework ${index + 1}`, titleZh: `官方作业 ${index + 1}`, url: tsinghuaAlgorithmSource, kind: "assignment" as const }]);
