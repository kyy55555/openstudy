...
function princetonCos333Tasks(): PlanTask[] {
  ...
  const tasks = weeks.flatMap((title, index) => {
    ...
    if (assignment) result.push({ id: `assignment-${assignment}`, title: `Registrar Application ${assignment}`, titleZh: `教务系统应用作业 ${assignment}`, url: "https://www.cs.princeton.edu/courses/archive/spring26/cos333/assignments.html", kind: "project" });
...
function princetonFoundationTasks(courseId: "mat103" | "mat201" | "mat202"): PlanTask[] {
  const definitions = {
...
for (let index = 1; index <= 5; index += 1) harvardCs1610Tasks.push({ id: `section-${index}`, title: `Official systems section ${index}`, titleZh: `官方系统专题讨论 ${index}`, url: harvardCs1610Source, kind: "session" });
for (let index = 1; index <= 5; index += 1) harvardCs1610Tasks.push({ id: `problem-set-${index}`, title: `Official Chickadee problem set ${index}`, titleZh: `官方 Chickadee 作业 ${index}`, url: `https://read.seas.harvard.edu/cs1610/2026/pset${index}/`, kind: "project", resourceType: "assignments" });
harvardCs1610Tasks.push({ id: "midterm-1", title: "Official in-class midterm 1", titleZh: "官方课堂期中考试一", url: harvardCs1610Source, kind: "exam" }, { id: "midterm-2", title: "Official in-class midterm 2", titleZh: "官方课堂期中考试二", url: harvardCs1610Source, kind: "exam" });
structuredCoursePlans["harvard-cs1610"] = { sourceUrl: harvardCs1610Source, detail: "full", tasks: harvardCs1610Tasks };
const harvardCs1650Source = "https://stratos.seas.harvard.edu/class/cs165-data-systems";
const harvardCs1650Topics = [["Relational Algebra and SQL", "关系代数与 SQL"], ["C Systems Programming", "C 系统编程"], ["Hash Tables and Project Foundations", "哈希表与项目基础"], ["Memory Mapping and Storage", "内存映射与存储"], ["Memory Hierarchy and Caches", "内存层次与缓存"], ["Cache-Conscious Design", "缓存感知设计"], ["Processes, Virtual Memory, and the TLB", "进程、虚拟内存与 TLB"], ["Serialization", "序列化"], ["Database Catalogs and Metadata", "数据库目录与元数据"], ["System Profiling and Performance", "系统性能分析"], ["Parallelization and Synchronization", "并行化与同步"], ["ACID Across Database Evolution", "数据库演进中的 ACID"], ["Locking and Transactional Consistency", "锁与事务一致性"], ["Logging and Crash Recovery", "日志与崩溃恢复"], ["Spatial Databases and R-Trees", "空间数据库与 R 树"], ["Cost Modeling", "成本建模"], ["Column Stores and Access Methods", "列式存储与访问方法"], ["Modern Hardware-Aware Data-System Design", "现代硬件感知数据系统设计"]] as const;
const harvardCs1650Tasks: PlanTask[] = harvardCs1650Topics.map(([title, titleZh], index) => ({ id: `official-section-${index + 1}`, title: `Official class or recorded section ${index + 1}: ${title}`, titleZh: `官方课堂或录制专题 ${index + 1}：${titleZh}`, url: harvardCs1650Source, kind: "session", resourceType: "lectures" }));
harvardCs1650Tasks.push({ id: "project-0", title: "Official Project 0: hash-table preparation", titleZh: "官方项目零：哈希表准备", url: harvardCs1650Source, kind: "project" });
[["Storage Layer", "存储层"], ["Main-Memory Indexing", "内存索引"], ["Shared Scans", "共享扫描"], ["Joins", "连接运算"], ["Updates", "更新"]].forEach(([title, titleZh], index) => harvardCs1650Tasks.push({ id: `project-milestone-${index + 1}`, title: `Official project milestone ${index + 1}: ${title}`, titleZh: `官方项目里程碑 ${index + 1}：${titleZh}`, url: harvardCs1650Source, kind: "project", resourceType: "projects" }));
harvardCs1650Tasks.push({ id: "midterm-1", title: "Official take-home midterm 1", titleZh: "官方带回家期中考试一", url: harvardCs1650Source, kind: "exam" }, { id: "midterm-2", title: "Official take-home midterm 2", titleZh: "官方带回家期中考试二", url: harvardCs1650Source, kind: "exam" });
structuredCoursePlans["harvard-cs1650"] = { sourceUrl: harvardCs1650Source, detail: "full", tasks: harvardCs1650Tasks };
const harvardCs1200Schedule = "https://docs.google.com/spreadsheets/d/1mnbqSTgJGc4nRCR9vSJrPSvmCyY8zVAWi88YkILdJEQ/edit?gid=1779133089";
const harvardCs1200Topics = [["Sorting Algorithms", "排序算法"], ["Computational Problems and Measuring Efficiency", "计算问题与效率度量"], ["Complexity of Sorting", "排序复杂度"], ["Reductions", "归约"], ["Static and Dynamic Data Structures", "静态与动态数据结构"], ["Binary Search Trees", "二叉搜索树"], ["RAM, RAM Simulation, and Word-RAM", "RAM、RAM 模拟与 Word-RAM"], ["RAM versus Word-RAM and Church–Turing", "RAM、Word-RAM 与 Church–Turing 论题"], ["Randomized Algorithms and QuickSelect", "随机算法与 QuickSelect"], ["Randomized Data Structures", "随机数据结构"], ["Graphs and Graph Search", "图与图搜索"], ["Independent Sets", "独立集"], ["Graph Coloring and Connected Components", "图着色与连通分量"], ["Matching", "匹配"], ["Matching and Ethics", "匹配与伦理"], ["Logic", "逻辑"], ["Resolution", "归结"], ["Resolution and Computational Complexity", "归结与计算复杂度"], ["NP Search, NP, and NP-Completeness", "NP 搜索、NP 与 NP 完全性"], ["NP and NP-Completeness", "NP 与 NP 完全性"], ["P versus NP and Universal Programs", "P 与 NP 以及通用程序"], ["Unsolvable Problems", "不可解问题"], ["Course Synthesis", "课程总结"], ["Cook–Levin Theorem", "Cook–Levin 定理"]] as const;
const harvardCs1200Tasks: PlanTask[] = harvardCs1200Topics.map(([title, titleZh], index) => ({ id: `lecture-${index + 1}`, title: `Official lecture ${index + 1}: ${title}`, titleZh: `官方讲次 ${index + 1}：${titleZh}`, url: harvardCs1200Schedule, kind: "session", resourceType: "lectures" }));
for (let index = 0; index <= 10; index += 1) harvardCs1200Tasks.push({ id: `problem-set-${index}`, title: `Official problem set ${index} with published solution`, titleZh: `官方习题集 ${index} 与公开答案`, url: harvardCs1200Schedule, kind: "assignment", resourceType: "assignments" });
harvardCs1200Tasks.push({ id: "midterm", title: "Official midterm and published solutions", titleZh: "官方期中考试与公开答案", url: harvardCs1200Schedule, kind: "exam" }, { id: "final", title: "Official final examination", titleZh: "官方期末考试", url: harvardCs1200Schedule, kind: "exam" });
structuredCoursePlans["harvard-cs1200"] = { sourceUrl: harvardCs1200Schedule, detail: "full", tasks: harvardCs1200Tasks };

const cmu15445Archive = "https://15445.courses.cs.cmu.edu/fall2025";
const cmu15445Lectures = [
  ["Relational Model & Algebra", "关系模型与关系代数", "7NPIENPr-zk"], ["Modern SQL", "现代 SQL", "O5gU9NQjCAs"], ["Database Storage I", "数据库存储（一）", "PRLXdIMJhOg"], ["Memory Management", "内存管理", "8-2yv4z0VZc"], ["Database Storage II", "数据库存储（二）", "2_sTdS4h-bY"], ["Storage Models & Compression", "存储模型与压缩", "yWnToWrskXE"], ["Hash Tables", "哈希表", "nuNW8IfgPNU"], ["Indexes & Filters I", "索引与过滤器（一）", "u7ii_Lvm9rM"], ["Indexes & Filters II", "索引与过滤器（二）", "PjST2n7abAY"], ["Index Concurrency Control", "索引并发控制", "YgOvfXl6pss"], ["Sorting & Aggregation Algorithms", "排序与聚合算法", "LzyKTpeIgts"], ["Join Algorithms", "连接算法", "YIdIaPopfpk"], ["Query Execution I", "查询执行（一）", "E-UUd6cB57w"], ["Query Execution II", "查询执行（二）", "Kzf1hGjtZOU"], ["Query Planning & Optimization I", "查询规划与优化（一）", "b53huOGcsZ8"], ["Query Planning & Optimization II", "查询规划与优化（二）", "azTHRpzl10o"], ["Concurrency Control Theory", "并发控制理论", "tMFAgvDViAI"], ["Two-Phase Locking", "两阶段锁", "drStlhNbfHI"], ["Timestamp Ordering", "时间戳排序", "risHwKeWbBM"], ["Multi-Version Concurrency Control", "多版本并发控制", "tUFha9-DuSk"], ["Database Logging", "数据库日志", "CedEy54pe3g"], ["Database Recovery", "数据库恢复", "X2jc4qalNy0"], ["Distributed Databases I", "分布式数据库（一）", "IFLQBWY6dlE"], ["Distributed Databases II", "分布式数据库（二）", "pQh5fka3FC0"], ["Final Review & Systems Potpourri", "期末复习与系统专题", "qiVUf9X6ItM"],
] as const;
const cmu15445Work = [
  ["homework-sql", "Homework: SQL", "作业：SQL", "homework1/", "assignment"], ["homework-storage", "Homework: Storage", "作业：存储", "files/hw2-clean.pdf", "assignment"], ["homework-indexes", "Homework: Indexes & Filters", "作业：索引与过滤器", "files/hw3-clean.pdf", "assignment"], ["homework-execution", "Homework: Execution & Planning", "作业：执行与规划", "files/hw4-clean.pdf", "assignment"], ["homework-transactions", "Homework: Transactions", "作业：事务", "files/hw5-clean.pdf", "assignment"], ["homework-recovery", "Homework: Recovery", "作业：恢复", "files/hw6-clean.pdf", "assignment"], ["project-cpp", "Project: C++ Primer", "项目：C++ 入门", "project0/", "project"], ["project-buffer-pool", "Project: Buffer Pool Manager", "项目：缓冲池管理器", "project1/", "project"], ["project-index", "Project: Database Index", "项目：数据库索引", "project2/", "project"], ["project-query", "Project: Query Execution", "项目：查询执行", "project3/", "project"], ["project-concurrency", "Project: Concurrency Control", "项目：并发控制", "project4/", "project"],
] as const;
function cmu15445Tasks(): PlanTask[] {
  const lectures: PlanTask[] = cmu15445Lectures.map(([title, titleZh, videoId], index) => ({ id: `lecture-${index + 1}`, title: `Lecture ${index + 1}: ${title}`, titleZh: `第 ${index + 1} 讲：${titleZh}`, url: `https://www.youtube.com/watch?v=${videoId}&list=PLSE8ODhjZXjYMAgsGH-GtY5rJYZ6zjsd5&index=${index + 1}`, kind: "session", resourceType: "lectures" }));
  const work: PlanTask[] = cmu15445Work.map(([id, title, titleZh, path, kind]) => ({ id, title, titleZh, url: `${cmu15445Archive}/${path}`, kind, resourceType: kind === "project" ? "projects" : "assignments" }));
  return [...lectures, ...work];
}

const cmu15362Base = "https://15362.courses.cs.cmu.edu/spring2026";
const cmu15362Lectures = [
  ["intro", "Introduction & the Graphics Pipeline", "图形学导论与图形流水线"], ["math", "Linear Algebra & Vector Calculus", "线性代数与向量微积分"], ["cpp", "C++: A Programmer's Perspective", "从程序员视角学习 C++"], ["transforms", "Coordinate Spaces & Transformations", "坐标空间与变换"], ["persp", "Perspective Projection", "透视投影"], ["bary", "Rasterization and Barycentric Coordinates", "光栅化与重心坐标"], ["depth", "Depth and Blending", "深度与混合"], ["midterm", "Midterm Review", "期中复习"], ["geom", "Introduction to Geometry", "几何导论"], ["dgp", "Digital Geometry Processing", "数字几何处理"], ["spatial", "Spatial Data Structures", "空间数据结构"], ["color", "Color", "颜色"], ["radiometry", "Radiometry", "辐射度量学"], ["renderEq", "The Rendering Equation", "渲染方程"], ["brdf", "Bidirectional Reflectance Distribution Function", "双向反射分布函数"], ["variance", "Variance Reduction", "方差缩减"], ["anim", "Principles of Animation", "动画原理"], ["splines", "Splines", "样条"], ["kine", "Kinematics", "运动学"], ["sim", "Simulation and ODEs", "仿真与常微分方程"], ["pdes", "Partial Differential Equations", "偏微分方程"], ["opt", "Optimization and More", "优化与进阶专题"], ["final", "Final Review", "期末复习"],
] as const;
const cmu15362Assignments = [
  ["assignment-0", "Assignment 0: Math and code review", "作业 0：数学与代码复习", "A0.md#assignment-0-scotty3d"], ["assignment-1-checkpoint", "Assignment 1 checkpoint: Rasterizer", "作业 1 检查点：光栅器", "A1.md#assignment-1-rasterizer"], ["assignment-1", "Assignment 1: Rasterizer", "作业 1：光栅器", "A1.md#assignment-1-rasterizer"], ["assignment-2-checkpoint", "Assignment 2 checkpoint: MeshEdit", "作业 2 检查点：网格编辑", "A2.md"], ["assignment-2", "Assignment 2: MeshEdit", "作业 2：网格编辑", "A2.md"], ["assignment-3-checkpoint", "Assignment 3 checkpoint: PathTracer", "作业 3 检查点：路径追踪", "A3.md"], ["assignment-3", "Assignment 3: PathTracer", "作业 3：路径追踪", "A3.md"], ["assignment-4-checkpoint", "Assignment 4 checkpoint: Animation", "作业 4 检查点：动画", "A4.md"], ["assignment-4", "Assignment 4: Animation", "作业 4：动画", "A4.md"],
] as const;
function cmu15362Tasks(): PlanTask[] {
  return [
    ...cmu15362Lectures.map(([slug, title, titleZh], index) => ({ id: `lecture-${index + 1}`, title: `Lecture ${index + 1}: ${title}`, titleZh: `第 ${index + 1} 讲：${titleZh}`, url: `${cmu15362Base}/lecture/${slug}`, kind: "session" as const, resourceType: "lectures" as const })),
    ...cmu15362Assignments.map(([id, title, titleZh, path]) => ({ id, title, titleZh, url: `https://github.com/CMU-Graphics/Scotty3D/blob/main/assignments/${path}`, kind: "assignment" as const, resourceType: "assignments" as const })),
  ];
}
...
const mitProductDevelopmentCalendar = "https://ocw.mit.edu/courses/15-783j-product-design-and-development-spring-2006/pages/calendar/";
const mitProductDevelopmentLectures = "https://ocw.mit.edu/courses/15-783j-product-design-and-development-spring-2006/pages/lecture-notes/";
const mitProductDevelopmentAssignments = "https://ocw.mit.edu/courses/15-783j-product-design-and-development-spring-2006/pages/assignments/";
const mitProductDevelopmentProjects = "https://ocw.mit.edu/courses/15-783j-product-design-and-development-spring-2006/pages/projects/";
const mitProductDevelopmentTopics = [["Introduction and Project Selection", "导论与项目选择"], ["The Product Development Process", "产品开发流程"], ["Project Proposals", "项目提案"], ["Identifying Customer Needs", "识别用户需求"], ["Customer Preferences and Conjoint Analysis", "用户偏好与联合分析"], ["Industrial Design", "工业设计"], ["Product Specifications and Concept Generation", "产品规格与概念生成"], ["Prototyping", "原型设计"], ["Concept Selection", "概念筛选"], ["Faculty Product Consulting I", "产品咨询（一）"], ["Faculty Product Consulting II", "产品咨询（二）"], ["Intellectual Property", "知识产权"], ["Proof-of-Concept Consulting", "概念验证咨询"], ["Peer Concept Review I", "同行概念评审（一）"], ["Peer Concept Review II", "同行概念评审（二）"], ["Product Architecture", "产品架构"], ["Forecasting Demand for New Products", "新产品需求预测"], ["Global Manufacturing and Costing", "全球制造与成本"], ["Design for Manufacturing", "面向制造的设计"], ["Trends and Tools in Product Development", "产品开发趋势与工具"], ["Final Project Consulting I", "最终项目咨询（一）"], ["Final Project Consulting II", "最终项目咨询（二）"], ["Final Product Presentation", "最终产品展示"]] as const;
const mitProductDevelopmentTasks: PlanTask[] = mitProductDevelopmentTopics.map(([title, titleZh], index) => ({ id: `session-${index + 1}`, title: `Official session ${index + 1}: ${title}`, titleZh: `官方第 ${index + 1} 讲：${titleZh}`, url: index === 0 || [2, 9, 10, 12, 13, 14, 20, 21, 22].includes(index) ? mitProductDevelopmentCalendar : mitProductDevelopmentLectures, kind: "session", resourceType: "lectures" }));
[["Project Proposal", "项目提案"], ["Mission Statement and Customer Needs", "使命陈述与用户需求"], ["Concept Sketches, Specifications, and Patent Review", "概念草图、规格与专利检索"], ["Concept Selection and Development Schedule", "概念筛选与开发进度"], ["Final Concept and Proof-of-Concept Model", "最终概念与概念验证模型"], ["Prototype Drawings, Bill of Materials, and Schedule", "原型图纸、物料清单与进度"], ["Product Financial Model", "产品财务模型"], ["Alpha Prototype", "Alpha 原型"], ["Final Presentation and Demonstration", "最终展示与演示"]].forEach(([title, titleZh], index) => mitProductDevelopmentTasks.push({ id: `assignment-${index + 1}`, title: `Official assignment ${index + 1}: ${title}`, titleZh: `官方作业 ${index + 1}：${titleZh}`, url: mitProductDevelopmentAssignments, kind: index >= 7 ? "project" : "assignment", resourceType: index >= 7 ? "projects" : "assignments" }));
mitProductDevelopmentTasks.push({ id: "project-examples", title: "Review official completed product examples", titleZh: "复盘官方完整产品案例", url: mitProductDevelopmentProjects, kind: "project", resourceType: "projects" });
structuredCoursePlans["mit-15-783j"] = { sourceUrl: mitProductDevelopmentCalendar, detail: "full", tasks: mitProductDevelopmentTasks };

const mitNewEnterprisesCalendar = "https://ocw.mit.edu/courses/15-390-new-enterprises-spring-2013/pages/calendar/";
const mitNewEnterprisesLectures = "https://ocw.mit.edu/courses/15-390-new-enterprises-spring-2013/pages/lecture-notes/";
const mitNewEnterprisesAssignments = "https://ocw.mit.edu/courses/15-390-new-enterprises-spring-2013/pages/assignments/";
const mitNewEnterprisesTopics = [["Course Introduction and Business Plans", "课程导论与商业计划"], ["Idea Generation", "创意生成"], ["Analyzing Ideas and Inspirations", "创意与灵感分析"], ["Team Formation", "团队组建"], ["Idea Refinement", "创意打磨"], ["Market Segmentation and Primary Research", "市场细分与一手调研"], ["Selecting a Target Market", "选择目标市场"], ["Customer Value Proposition", "用户价值主张"], ["Defining a Unique Advantage", "定义独特优势"], ["Marketing Plan Simulation", "营销计划模拟"], ["Competitive Advantage", "竞争优势"], ["Customer Acquisition Cost", "用户获取成本"], ["Customer Acquisition Examples", "获客案例"], ["Sustainable Business Economics", "可持续商业经济模型"], ["Sales and Distribution", "销售与分销"], ["Go-to-Market Simulation", "市场进入模拟"], ["Designing and Building the Product", "设计并构建产品"], ["Financial Statements", "财务报表"], ["Product Definition, Development, and Delivery", "产品定义、开发与交付"], ["Legal Considerations", "法律事项"], ["Scaling the Business and Presenting the Plan", "业务规模化与计划展示"], ["Managing Troubles Ahead", "应对后续风险"], ["Venture Presentation I", "创业项目展示（一）"], ["Venture Presentation II", "创业项目展示（二）"]] as const;
const mitNewEnterprisesTasks: PlanTask[] = mitNewEnterprisesTopics.map(([title, titleZh], index) => ({ id: `session-${index + 1}`, title: `Official session ${index + 1}: ${title}`, titleZh: `官方第 ${index + 1} 讲：${titleZh}`, url: [9, 15, 22, 23].includes(index) ? mitNewEnterprisesCalendar : mitNewEnterprisesLectures, kind: "session", resourceType: "lectures" }));
[["Ideas, Elevator Pitch, and Team Formation", "创意、电梯演讲与团队组建"], ["Executive Summary", "执行摘要"], ["Target Customer and Value Proposition", "目标用户与价值主张"], ["Market Definition", "市场定义"], ["Competitive Advantage", "竞争优势"], ["Customer Acquisition Analysis", "获客分析"], ["Go-to-Market, Sales, and Pricing", "市场进入、销售与定价"], ["Product Section", "产品章节"], ["Financials and Human Resources", "财务与人力资源"], ["Executive Summary Revision", "执行摘要修订"], ["Founder Interview", "创业者访谈"], ["Final Venture Paper", "最终创业报告"]].forEach(([title, titleZh], index) => mitNewEnterprisesTasks.push({ id: `assignment-${index + 1}`, title: `Official assignment ${index + 1}: ${title}`, titleZh: `官方作业 ${index + 1}：${titleZh}`, url: mitNewEnterprisesAssignments, kind: index === 11 ? "project" : "assignment", resourceType: index === 11 ? "projects" : "assignments" }));
structuredCoursePlans["mit-15-390"] = { sourceUrl: mitNewEnterprisesCalendar, detail: "full", tasks: mitNewEnterprisesTasks };
